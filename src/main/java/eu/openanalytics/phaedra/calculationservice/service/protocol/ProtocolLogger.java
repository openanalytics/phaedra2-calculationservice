...
public class ProtocolLogger {
    ...
    public static void log(Logger logger, CalculationContext cctx, String message, Object... formatArgs) {
        var prefix = String.format("Calculation [ResultSetId: %s, ProtocolId: %s, PlateId %s, MeasId: %s] ",  cctx.getResultSetId(), cctx.getProtocol().getId(), cctx.getPlate().getId(), cctx.getMeasId());
